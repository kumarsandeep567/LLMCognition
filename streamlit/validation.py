...
def display_validation_page():
    ...
    if 'task_id' in st.session_state and 'user_id' in st.session_state:
    ...
    if 'action' in st.session_state:
        if st.session_state['action'] == True:
            ...
            response = query_gpt(task_id, user_id)
    ...
    with col2:
        # Place the back button in the top right corner
        if st.button("Back"):
    ...
    if st.button("Update and Regenerate GPT Response"):
        new_response = query_gpt(task_id, user_id, updated_steps=updated_annotation_steps)
