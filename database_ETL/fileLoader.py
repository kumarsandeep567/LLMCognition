...
def load_files_into_gcp(repository_id, repository_type, validation_files, bucket_name, creds_file_path, file_path, gcp_folder_path):
    ...
    try:
        ...
        for file in validation_files:
            ...
            gcs_file_path = os.path.join(gcp_folder_path, os.path.basename(file))
...
def load_files(access_token, repository_id, repository_type, file_path):
    ...
    validation_files = [file for file in files if file.startswith(file_path)]
...
def main():
    ...
    validation_files = load_files(access_token, repository_id, repository_type, file_path)
    load_files_into_gcp(repository_id, repository_type, validation_files, bucket_name, creds_file_path, file_path, gcp_folder_path)
